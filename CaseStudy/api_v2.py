...
def retrive_sensor_data(host, user, password, database, start_dt, end_dt):
...
def retrieve_meteo_data(start_dt, end_dt):
...
def forecast():
    ...
    start_dt = str(date_now_round - timedelta(hours=47))

    rows = retrive_sensor_data(
        host="localhost",
        user="root",
        password="password",
        database="sensors", 
    ...
    h = retrieve_meteo_data(start_dt = start_dt.split(' ')[0], end_dt = end_dt.split(' ')[0])
    print(h)
    print('-'*20)
    features = ["temperature_2m","relative_humidity_2m","cloud_cover","wind_speed_10m","wind_direction_100m","soil_temperature_0_to_7cm","soil_temperature_7_to_28cm","soil_temperature_28_to_100cm"]
    ...
    h[features] = h[features].fillna(0)
    ...
    for i in range(0,48):
        r  = rows[i]
        a = [r['s_a'], r['s_b'], r['irr'], r['LAI']]
        b = h.iloc[i][features].to_list()
        a.extend(b)
        x.append(a)
        print(b)
